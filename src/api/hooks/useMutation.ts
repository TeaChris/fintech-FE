...
export function useApiMutation<TData, TVariables = void>(
      ...
      const mutationOptions: UseMutationOptions<TData, Error, TVariables> = {
            mutationFn: async (variables: TVariables): Promise<TData> => {
                  ...
                  switch (method) {
                        case 'POST':
                              response = await client.post<TData>(
                                    path,
                                    requestConfig,
                              )
                              break
                        case 'PUT':
                              response = await client.put<TData>(
                                    path,
                                    requestConfig,
                              )
                              break
                        case 'PATCH':
                              response = await client.patch<TData>(
                                    path,
                                    requestConfig,
                              )
                              break
                        case 'DELETE':
                              response = await client.del<TData>(
                                    path,
                                    requestConfig,
                              )
                              break
                        default: {
                              ...
                              throw new Error(
                                    `Unsupported mutation method: ${_exhaustive}`,
                              )
                        }
                  }
            ...
            onError: (error, variables, context) => {
                  ...
                  if (
                        ...
                        queryClient.setQueryData(
                              optimistic.queryKey,
                              (context as { previousData: unknown })
                                    .previousData,
                        )
                  }
            ...
            onSuccess: async (data, variables) => {
                  ...
                  if (isFinancialMutation) {
                        stableIdempotencyKeyRef.current =
