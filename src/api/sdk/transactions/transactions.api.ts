...
import type { Transaction, TransactionList, TransactionFilters } from '@/api/schemas';
...
export interface TransactionsApi {
  ...
  listByAccount(accountId: string, filters?: Omit<TransactionFilters, 'accountId'>): Promise<ApiResponse<TransactionList>>;
}
...
export function createTransactionsApi(client: ApiClient): TransactionsApi {
  return {
    ...
    listByAccount: (accountId, filters) =>
      client.get<TransactionList>('/accounts/{accountId}/transactions', {
        params: { accountId },
        query: filters as Record<string, string | number | boolean | undefined>,
        schema: TransactionListSchema as unknown as ZodType,
      }),
