...
export interface CardsApi {
  ...
  list(filters?: { status?: string; type?: string; accountId?: string }): Promise<ApiResponse<CardList>>;
  block(cardId: string, reason: string): Promise<ApiResponse<Card>>;
  activate(data: { cardId: string; pin: string; cvv: string; expiryMonth: number; expiryYear: number }): Promise<ApiResponse<Card>>;
}
