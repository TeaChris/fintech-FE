...
export class NetworkError extends ApiError {
  ...
  constructor(message: string, meta: Partial<ApiErrorMeta> & { requestId: string; correlationId: string }) {
    super(message, {
      status: 0,
      method: 'GET' as const,
      url: '',
...
export class RateLimitError extends ApiError {
  ...
  constructor(message: string, meta: ApiErrorMeta, retryAfterSeconds: number = 60) {
  ...
  switch (response.status) {
    ...
    case 429: {
      ...
      const retryAfterSeconds = retryAfterHeader
        ...
        : 60;
      return new RateLimitError(message, baseMeta, retryAfterSeconds);
    }

    default: {
      // Check for fraud review (custom status or code)
      if (body.code === 'FRAUD_REVIEW' || response.status === 451) {
        const reviewId = typeof body.details?.['reviewId'] === 'string'
          ? body.details['reviewId']
          : undefined;
        return new FraudReviewError(message, baseMeta, reviewId);
      }

      // All 5xx errors are treated as server errors
      if (response.status >= 500) {
        return new UnknownServerError(message, { ...baseMeta, retryable: true });
      }
